...
namespace stsc
{
	namespace engine
	{
		namespace algorithms_storage
		{
			...
			class on_bar_algorithm : public algorithm_prototype< common::on_bar, output_signal_type >
			{
				typedef typename algorithm_prototype< common::on_bar, signal_type > base_class;
			protected:
				explicit on_bar_algorithm( const std::string& name, typed_serie_ptr& serie );
			...
			template< typename output_signal_type >
			on_bar_algorithm< output_signal_type >::on_bar_algorithm( const std::string& name, typed_serie_ptr& serie )
				: base_class( algorithm_storage().register_algorithm_name( name ), serie )
			{
				algorithm_storage().register_on_bar( name_, this );
			}
			template< typename output_signal_type >
